...
use super::{
    bind_media_parts,
    image::{rescale_and_normalize_rgb8, resize_rgb8_bicubic, NormalizedImage},
...
impl Gemma4Processor {
    pub(super) fn load(model_dir: &Path) -> Result<Option<Self>, Error> {
        ...
        let Some(vision) = config.vision_config else {
            return Ok(None);
        };
        ...
        Ok(Some(Self {
            patch_size: processor.patch_size.unwrap_or(vision.patch_size),
            pooling_kernel_size: processor
                .pooling_kernel_size
                .unwrap_or(vision.pooling_kernel_size),
            max_soft_tokens,
            image_token_id: config.image_token_id.ok_or_else(|| {
                Error::Processor("Gemma 4 image processor requires image_token_id".into())
            })?,
            boi_token_id: config.boi_token_id.ok_or_else(|| {
                Error::Processor("Gemma 4 image processor requires boi_token_id".into())
            })?,
            eoi_token_id: config.eoi_token_id.ok_or_else(|| {
    ...
    pub(super) fn prepare_token_ids(
        ...
        for item in media {
            let (Modality::Image, MediaPayload::Rgb8(image)) = (item.modality, item.payload) else {
                return Err(Error::Processor(format!(
                    "Gemma 4 processor does not support {} media yet",
                    item.modality.as_str()
                )));
            };
            bindings.push(PreparedMediaBinding {
                placeholder_token_id: self.image_token_id,
                prefix_token_ids: vec![self.boi_token_id],
                suffix_token_ids: vec![self.eoi_token_id],
                part: self.process_image(image)?,
            });
        }
        bind_media_parts(token_ids, &[self.image_token_id], bindings)
    }
