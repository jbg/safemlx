...
fn test_ops_arithmetic_abs() {
    ...
    let result = safemlx::abs!(&data).unwrap();
...
fn test_ops_arithmetic_add() {
    ...
    let result = safemlx::add!(&data1, &data2).unwrap();
...
fn test_ops_arithmetic_tensordot() {
    ...
    let axes_x = [1, 0];
    let axes_y = [0, 1];
    let z = safemlx::tensordot_axes!(&x, &y, &axes_x, &axes_y).unwrap();
...
fn test_ops_factory_arange() {
    ...
    let array = safemlx::arange!(stop = 50).unwrap();
    ...
    let array = safemlx::arange!(start = 1.0, stop = 50.0, step = 2.0).unwrap();
...
fn test_fft_fft() {
    ...
    let fft = safemlx::fft!(&data).unwrap();
...
fn test_linalg_norm() {
    ...
    let norm = safemlx::norm_l2!(&a).unwrap();
...
fn test_random_uniform() {
    let value = safemlx::uniform!(0.0, 1.0, shape = &[1]).unwrap();
...
fn test_random_normal() {
    let value = safemlx::normal!(shape = &[1]).unwrap();
    assert_eq!(value.shape(), &[1]);
...
fn test_fast_sdpa_using_macros() -> Result<(), Exception> {
    ...
    for seq_len in [63, 129, 400] {
        for dtype in [crate::Dtype::Float32, crate::Dtype::Float16] {
            let B = 2;
            let H = 24;
            let q = safemlx::normal!(shape = &[B, H, seq_len, Dk])?.as_dtype(dtype)?;
            let k = safemlx::normal!(shape = &[B, H, seq_len, Dk])?.as_dtype(dtype)?;
            let v = safemlx::normal!(shape = &[B, H, seq_len, Dk])?.as_dtype(dtype)?;

            let result = safemlx::scaled_dot_product_attention!(q, k, v, scale)?;
