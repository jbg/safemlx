...
use safemlx::{ops, transforms, Array};
use std::error::Error;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // True weight vector
    // let w_star = safemlx::random::normal::<f32>(&[num_features], None, None, None)?;
    let w_star = safemlx::normal!(shape = &[num_features])?;

    // Input examples (design matrix)
    // let x = safemlx::random::normal::<f32>(&[num_examples, num_features], None, None, None)?;
    let x = safemlx::normal!(shape = &[num_examples, num_features])?;

    // Noisy labels
    // let eps = safemlx::random::normal::<f32>(&[num_examples], None, None, None)? * 1e-2;
    let eps = safemlx::normal!(shape = &[num_examples])? * 1e-2;
    let y = x.matmul(&w_star)? + eps;

    // Initialize random weights
    // let w = safemlx::random::normal::<f32>(&[num_features], None, None, None)? * 1e-2;
    let w = safemlx::normal!(shape = &[num_features])? * 1e-2;
